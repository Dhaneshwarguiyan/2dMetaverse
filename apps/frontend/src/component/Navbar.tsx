  ...
  return (
    <div>
        ...
        </div>
        {
          ...
          <span onClick={()=>{navigate('/login')}}><Button text="Sign in" type="primary"/></span>
          <span onClick={()=>{navigate('/signup')}}><Button text="Sign up" type="secondary"/></span>
      </div> :
              <div className='flex items-center gap-2 relative cursor-pointer' onClick={()=>{setIsActive(!isActive)}} ref={dropDownRef}>
              <span className='text-gray-500'>{user.info?.username}</span>
              <img src={profile} alt="profile" className='w-[35px]' />
              {
                isActive &&
                <span className='absolute top-10 right-0'><ProfileDropDown /></span>
              }
            </div>
        }
        </div>
